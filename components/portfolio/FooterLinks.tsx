...
function getIcon(href: string, label: string) {
    if (href.startsWith("mailto:")) return <EmailIcon />;
    if (label === "GitHub") return <GitHubIcon />;
    if (label === "LinkedIn") return <LinkedInIcon />;
    if (label === "X") return <XIcon />;
    if (label === "LeetCode") return <LeetCodeIcon />;
    return <EmailIcon />;
}

export default function FooterLinks({ links }: { links: SocialLink[] }) {
    return (
